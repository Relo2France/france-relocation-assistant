<?php
/**
 * MyTravelStatus Location Handling
 *
...
class MTS_Location {
	 ...
	 * @return MTS_Location
	 */
	public static function get_instance(): MTS_Location {
	...
	public function store_location_debug( $request ) {
		...
		$table = $wpdb->prefix . 'mts_location_log';
	...
	public function check_location_permission( WP_REST_Request $request ) {
		...
		if ( ! $location ) {
			return new WP_Error(
				'not_found',
				__( 'Location entry not found.', 'mytravelstatus' ),
				array( 'status' => 404 )
			);
		}
		...
		if ( (int) $location->user_id !== get_current_user_id() ) {
			return new WP_Error(
				'forbidden',
				__( 'You do not have permission to access this location entry.', 'mytravelstatus' ),
				array( 'status' => 403 )
			);
		}
	...
	private function get_location_by_id( int $location_id ) {
		global $wpdb;
		$table = MTS_Schema::get_table( 'location_log' );
	...
	public function store_location( WP_REST_Request $request ) {
		...
		if ( $lat < -90 || $lat > 90 ) {
			return new WP_Error(
				'invalid_latitude',
				__( 'Latitude must be between -90 and 90.', 'mytravelstatus' ),
				array( 'status' => 400 )
			);
		}
		if ( $lng < -180 || $lng > 180 ) {
			return new WP_Error(
				'invalid_longitude',
				__( 'Longitude must be between -180 and 180.', 'mytravelstatus' ),
				array( 'status' => 400 )
			);
		}
		...
		if ( false === $result ) {
			return new WP_Error(
				'db_error',
				__( 'Failed to store location.', 'mytravelstatus' ),
				array( 'status' => 500 )
			);
		}
		 ...
		 */
		do_action( 'mts_location_recorded', $user_id, $location, $is_schengen );
		...
		return rest_ensure_response( array(
			...
			'message'  => $is_schengen
				? sprintf(
					...
					__( 'Location recorded in %s (Schengen zone).', 'mytravelstatus' ),
					$country_name
				)
				: sprintf(
					/* translators: %s: country name */
					__( 'Location recorded in %s.', 'mytravelstatus' ),
					$country_name ?: __( 'Unknown location', 'mytravelstatus' )
				),
		) );
	}
	...
	private function perform_reverse_geocode( float $lat, float $lng ): array {
		...
		if ( empty( $data ) || isset( $data['error'] ) ) {
			return array(
				'error' => isset( $data['error'] ) ? $data['error'] : __( 'Geocoding failed.', 'mytravelstatus' ),
			);
		}
	...
	public function delete_location( WP_REST_Request $request ) {
		...
		if ( false === $result ) {
			return new WP_Error(
				'db_error',
				__( 'Failed to delete location.', 'mytravelstatus' ),
				array( 'status' => 500 )
			);
		}
	...
	public function clear_location_history( WP_REST_Request $request ) {
		...
		if ( false === $result ) {
			return new WP_Error(
				'db_error',
				__( 'Failed to clear location history.', 'mytravelstatus' ),
				array( 'status' => 500 )
			);
		}
		 ...
		 * @param int $user_id User ID.
		 */
		do_action( 'mts_location_history_cleared', $user_id );

		return rest_ensure_response( array(
			'cleared' => true,
			'message' => __( 'Location history has been cleared.', 'mytravelstatus' ),
		) );
	}
	...
	public function update_location_settings( WP_REST_Request $request ) {
		...
		update_user_meta( $user_id, 'mts_location_settings', $settings );
	...
	private function get_user_location_settings( int $user_id ): array {
		...
		$saved = get_user_meta( $user_id, 'mts_location_settings', true );
	...
	public function detect_from_ip( WP_REST_Request $request ) {
		...
		if ( ! $ip || $ip === '127.0.0.1' || strpos( $ip, '192.168.' ) === 0 || strpos( $ip, '10.' ) === 0 ) {
			return rest_ensure_response( array(
				'detected'    => false,
				'reason'      => 'local_ip',
				'message'     => __( 'Cannot detect location from local IP address.', 'mytravelstatus' ),
				'countryCode' => null,
				'countryName' => null,
				'city'        => null,
				'isSchengen'  => false,
		...
		if ( is_wp_error( $response ) ) {
			return rest_ensure_response( array(
				'detected'    => false,
				'reason'      => 'api_error',
				'message'     => __( 'Failed to detect location from IP.', 'mytravelstatus' ),
				'countryCode' => null,
				'countryName' => null,
				'city'        => null,
				'isSchengen'  => false,
		...
		if ( empty( $data ) || ( isset( $data['status'] ) && 'fail' === $data['status'] ) ) {
			return rest_ensure_response( array(
				'detected'    => false,
				'reason'      => 'lookup_failed',
				'message'     => $data['message'] ?? __( 'IP lookup failed.', 'mytravelstatus' ),
				'countryCode' => null,
				'countryName' => null,
				'city'        => null,
				'isSchengen'  => false,
		...
		$result = array(
			...
			'message'     => sprintf(
				/* translators: %s: country name */
				__( 'Detected location: %s', 'mytravelstatus' ),
				$data['country'] ?? __( 'Unknown', 'mytravelstatus' )
			),
			'countryCode' => $country_code,
			'countryName' => $data['country'] ?? null,
			'city'        => $data['city'] ?? null,
