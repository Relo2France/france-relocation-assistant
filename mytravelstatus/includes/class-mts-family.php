<?php
/**
 * Family Members API for MyTravelStatus.
 *
...
class MTS_Family {
	 ...
	 * @return MTS_Family
	 */
	public static function get_instance(): MTS_Family {
	...
	public function get_family_members( WP_REST_Request $request ): WP_REST_Response {
		...
		$table   = MTS_Schema::get_table( 'family_members' );
	...
	public function create_family_member( WP_REST_Request $request ) {
		...
		if ( false === $result ) {
			return new WP_Error(
				'db_error',
				__( 'Failed to create family member.', 'mytravelstatus' ),
				array( 'status' => 500 )
			);
		}
	...
	public function update_family_member( WP_REST_Request $request ) {
		...
		if ( empty( $data ) ) {
			return new WP_Error(
				'no_data',
				__( 'No data to update.', 'mytravelstatus' ),
				array( 'status' => 400 )
			);
		}
		...
		if ( false === $result ) {
			return new WP_Error(
				'db_error',
				__( 'Failed to update family member.', 'mytravelstatus' ),
				array( 'status' => 500 )
			);
		}
	...
	public function delete_family_member( WP_REST_Request $request ) {
		...
		if ( false === $result ) {
			return new WP_Error(
				'db_error',
				__( 'Failed to delete family member.', 'mytravelstatus' ),
				array( 'status' => 500 )
			);
		}
	...
	public function get_family_summary( WP_REST_Request $request ): WP_REST_Response {
		...
		$summary = array(
			'primary' => array(
				'name'   => __( 'Me (Primary)', 'mytravelstatus' ),
	...
	private function get_member_if_owner( int $member_id ) {
		...
		if ( ! $member ) {
			return new WP_Error(
				'not_found',
				__( 'Family member not found.', 'mytravelstatus' ),
				array( 'status' => 404 )
			);
		}
		...
		if ( (int) $member['user_id'] !== $user_id ) {
			return new WP_Error(
				'forbidden',
				__( 'You do not have permission to access this family member.', 'mytravelstatus' ),
				array( 'status' => 403 )
			);
		}
