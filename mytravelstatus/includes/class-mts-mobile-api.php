...
class MTS_Mobile_API {
	 ...
	 *
	 * @return MTS_Mobile_API
	 */
	public static function get_instance(): MTS_Mobile_API {
	...
	public function get_app_status( WP_REST_Request $request ): WP_REST_Response {
		...
		$maintenance_mode = get_option( 'mts_maintenance_mode', false );
		...
		$update_urls = array(
			'ios'     => get_option( 'mts_ios_app_url', '' ),
			'android' => get_option( 'mts_android_app_url', '' ),
		);
		...
		return rest_ensure_response( array(
			...
			'maintenance_message' => $maintenance_mode
				? get_option( 'mts_maintenance_message', __( 'The app is temporarily unavailable for maintenance.', 'mytravelstatus' ) )
				: null,
	...
	private function process_sync_change( int $user_id, array $change ) {
		...
		if ( ! in_array( $type, array( 'trip', 'location' ), true ) ) {
			return new WP_Error( 'invalid_type', __( 'Invalid change type.', 'mytravelstatus' ) );
		}

		if ( ! in_array( $action, array( 'create', 'update', 'delete' ), true ) ) {
			return new WP_Error( 'invalid_action', __( 'Invalid action.', 'mytravelstatus' ) );
		}
		...
		return new WP_Error( 'unknown_error', __( 'Unknown error processing change.', 'mytravelstatus' ) );
	}
	...
	private function process_trip_change( int $user_id, string $action, array $data ) {
		...
		if ( 'update' === $action ) {
			...
			if ( ! $existing ) {
				return new WP_Error( 'not_found', __( 'Trip not found.', 'mytravelstatus' ) );
			}

			if ( (int) $existing->user_id !== $user_id ) {
				return new WP_Error( 'forbidden', __( 'Access denied.', 'mytravelstatus' ) );
			}
			...
			if ( isset( $data['last_synced'] ) ) {
				...
				if ( $server_updated > $client_last_sync ) {
					return new WP_Error(
						'conflict',
						__( 'Conflict detected.', 'mytravelstatus' ),
						array(
							'type'           => 'trip',
							'id'             => $trip_id,
							'local_version'  => $data,
	...
	private function process_location_change( int $user_id, string $action, array $data ) {
		...
		return new WP_Error( 'unsupported', __( 'Only create action is supported for locations.', 'mytravelstatus' ) );
	}
	...
	public function register_device( WP_REST_Request $request ) {
		...
		if ( ! $device_id || ! $push_token || ! $platform ) {
			return new WP_Error(
				'missing_params',
				__( 'device_id, push_token, and platform are required.', 'mytravelstatus' ),
				array( 'status' => 400 )
			);
		}
		...
		if ( ! in_array( $platform, array( 'ios', 'android' ), true ) ) {
			return new WP_Error(
				'invalid_platform',
				__( 'Platform must be ios or android.', 'mytravelstatus' ),
				array( 'status' => 400 )
			);
		}
		...
		return rest_ensure_response( array(
			'success'   => true,
			'device_id' => $device_id,
			'message'   => __( 'Device registered successfully.', 'mytravelstatus' ),
		) );
	}
	...
	public function unregister_device( WP_REST_Request $request ) {
		...
		if ( ! $device_id ) {
			return new WP_Error(
				'missing_device_id',
				__( 'device_id is required.', 'mytravelstatus' ),
				array( 'status' => 400 )
			);
		}
		...
		if ( ! $device || (int) $device->user_id !== $user_id ) {
			return new WP_Error(
				'not_found',
				__( 'Device not found.', 'mytravelstatus' ),
				array( 'status' => 404 )
			);
		}
		...
		return rest_ensure_response( array(
			'success' => true,
			'message' => __( 'Device unregistered successfully.', 'mytravelstatus' ),
		) );
	}
	...
	public function upload_locations_batch( WP_REST_Request $request ) {
		...
		if ( empty( $locations ) ) {
			return new WP_Error(
				'no_locations',
				__( 'No locations provided.', 'mytravelstatus' ),
				array( 'status' => 400 )
			);
		}
