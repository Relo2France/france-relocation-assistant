...
class MTS_API {
	 ...
	 * @return MTS_API
	 */
	public static function get_instance(): MTS_API {
	...
	public function check_permission() {
		if ( ! is_user_logged_in() ) {
			return new WP_Error(
				'rest_forbidden',
				__( 'You must be logged in to access this resource.', 'mytravelstatus' ),
				array( 'status' => 401 )
			);
		}
		return true;
	...
	public function check_trip_permission( WP_REST_Request $request ) {
		...
		if ( ! $trip ) {
			return new WP_Error(
				'not_found',
				__( 'Trip not found.', 'mytravelstatus' ),
				array( 'status' => 404 )
			);
		}
		...
		if ( (int) $trip->user_id !== get_current_user_id() ) {
			return new WP_Error(
				'forbidden',
				__( 'You do not have permission to access this trip.', 'mytravelstatus' ),
				array( 'status' => 403 )
			);
		}
	...
	public function check_premium_permission() {
		...
		if ( ! $this->premium->is_feature_enabled( $user_id ) ) {
			return new WP_Error(
				'premium_required',
				__( 'This feature requires a premium subscription.', 'mytravelstatus' ),
				array( 'status' => 403 )
			);
		}
	...
	public function create_trip( WP_REST_Request $request ) {
		...
		if ( ! $this->premium->is_feature_enabled( $user_id ) ) {
			...
			if ( $trip_count >= self::FREE_TRIP_LIMIT ) {
				return new WP_Error(
					'trip_limit_reached',
					sprintf(
						__( 'Free tier is limited to %d trips. Upgrade to Premium for unlimited trips.', 'mytravelstatus' ),
		...
		if ( false === $result ) {
			return new WP_Error(
				'db_error',
				__( 'Failed to create trip.', 'mytravelstatus' ),
				array( 'status' => 500 )
			);
		}
		 ...
		 */
		do_action( 'mts_trip_created', $user_id, $this->format_trip( $trip ) );
	...
	public function update_trip( WP_REST_Request $request ) {
		...
		if ( empty( $data ) ) {
			return new WP_Error(
				'no_data',
				__( 'No data provided for update.', 'mytravelstatus' ),
				array( 'status' => 400 )
			);
		}
		...
		if ( false === $result ) {
			return new WP_Error(
				'db_error',
				__( 'Failed to update trip.', 'mytravelstatus' ),
				array( 'status' => 500 )
			);
		}
		 ...
		 */
		do_action( 'mts_trip_updated', get_current_user_id(), $this->format_trip( $trip ) );
	...
	public function delete_trip( WP_REST_Request $request ) {
		...
		if ( false === $result ) {
			return new WP_Error(
				'db_error',
				__( 'Failed to delete trip.', 'mytravelstatus' ),
				array( 'status' => 500 )
			);
		}
		 ...
		 */
		do_action( 'mts_trip_deleted', get_current_user_id(), $trip_id );
	...
	public function update_settings( WP_REST_Request $request ) {
		...
		if ( $yellow >= $red ) {
			return new WP_Error(
				'invalid_thresholds',
				__( 'Yellow threshold must be less than red threshold.', 'mytravelstatus' ),
				array( 'status' => 400 )
			);
		}
		...
		update_user_meta( $user_id, 'mts_settings', $settings );
	...
	public function get_feature_status( WP_REST_Request $request ) {
		...
		return rest_ensure_response( array(
			...
			'upgradeMessage' => $is_premium ? null : __( 'Upgrade to Premium for unlimited trips, planning tools, and PDF reports.', 'mytravelstatus' ),
		) );
	}
	...
	public function simulate_trip( WP_REST_Request $request ) {
		...
		if ( empty( $params['start_date'] ) || empty( $params['end_date'] ) ) {
			return new WP_Error(
				'missing_dates',
				__( 'Start date and end date are required.', 'mytravelstatus' ),
				array( 'status' => 400 )
			);
		}
	...
	public function test_alert( WP_REST_Request $request ) {
		...
		if ( ! class_exists( 'MTS_Alerts' ) ) {
			return new WP_Error(
				'alerts_unavailable',
				__( 'Schengen alerts system is not available.', 'mytravelstatus' ),
				array( 'status' => 500 )
			);
		}
	...
	public function import_trips_csv( WP_REST_Request $request ) {
		...
		if ( empty( $params['csv'] ) ) {
			return new WP_Error(
				'missing_csv',
				__( 'CSV data is required.', 'mytravelstatus' ),
				array( 'status' => 400 )
			);
		}
		...
		if ( count( $lines ) < 2 ) {
			return new WP_Error(
				'invalid_csv',
				__( 'CSV must contain a header row and at least one data row.', 'mytravelstatus' ),
				array( 'status' => 400 )
			);
		}
		...
		foreach ( $required as $col ) {
			if ( ! in_array( $col, $header, true ) ) {
				return new WP_Error(
					'missing_column',
					sprintf( __( 'Required column "%s" not found in CSV header.', 'mytravelstatus' ), $col ),
					array( 'status' => 400 )
				);
			}
		}
		...
		foreach ( $lines as $line_num => $line ) {
			...
			if ( empty( $start_date ) || ! preg_match( '/^\d{4}-\d{2}-\d{2}$/', $start_date ) ) {
				$errors[] = sprintf( __( 'Row %d: Invalid start_date format (expected YYYY-MM-DD).', 'mytravelstatus' ), $row_number );
				$skipped++;
				continue;
			}
			...
			if ( empty( $end_date ) || ! preg_match( '/^\d{4}-\d{2}-\d{2}$/', $end_date ) ) {
				$errors[] = sprintf( __( 'Row %d: Invalid end_date format (expected YYYY-MM-DD).', 'mytravelstatus' ), $row_number );
				$skipped++;
				continue;
			}
			...
			if ( is_wp_error( $validation ) ) {
				$errors[] = sprintf( __( 'Row %d: %s', 'mytravelstatus' ), $row_number, $validation->get_error_message() );
				$skipped++;
				continue;
			}
			...
			if ( ! $matched_country ) {
				$errors[] = sprintf( __( 'Row %d: Invalid country "%s".', 'mytravelstatus' ), $row_number, $country );
				$skipped++;
				continue;
			}
			...
			if ( false === $result ) {
				$errors[] = sprintf( __( 'Row %d: Database error.', 'mytravelstatus' ), $row_number );
				$skipped++;
			} else {
				$imported++;
			}
		...
		return rest_ensure_response( array(
			...
			'message'  => sprintf(
				__( 'Imported %d trips. %d rows skipped.', 'mytravelstatus' ),
				$imported,
				$skipped
			),
		) );
	...
	public function get_ai_suggestions( WP_REST_Request $request ) {
		...
		if ( $days_remaining > 0 ) {
			$suggestions[] = array(
				...
				'priority' => 'info',
				'icon'     => 'calendar',
				'title'    => sprintf( __( '%d days available', 'mytravelstatus' ), $days_remaining ),
				'message'  => sprintf(
					__( 'You can spend up to %d more days in the Schengen area without exceeding your 90-day limit.', 'mytravelstatus' ),
					$days_remaining
				),
			);
		} else {
			$suggestions[] = array(
				...
				'priority' => 'high',
				'icon'     => 'alert-triangle',
				'title'    => __( 'Limit reached', 'mytravelstatus' ),
				'message'  => __( 'You have used all 90 days in the current 180-day window. Wait for days to expire before planning new trips.', 'mytravelstatus' ),
			);
		}
		...
		if ( $next_expiration && $days_used > 0 ) {
			...
			if ( $days_until <= 30 ) {
				$suggestions[] = array(
					...
					'icon'     => 'clock',
					'title'    => sprintf(
						__( '%d day(s) expiring on %s', 'mytravelstatus' ),
						$expire_count,
						$expiry_date->format( 'M j, Y' )
					),
					'message'  => sprintf(
						__( 'In %d days, %d day(s) will drop off your 180-day window, giving you more availability.', 'mytravelstatus' ),
						$days_until,
						$expire_count
					),
				);
		...
		if ( $days_remaining > 0 ) {
			...
			if ( $optimal_trip ) {
				$suggestions[] = array(
					...
					'priority' => 'low',
					'icon'     => 'sparkles',
					'title'    => __( 'Optimal trip timing', 'mytravelstatus' ),
					'message'  => sprintf(
						__( 'For maximum flexibility, consider starting your next trip on %s. You could stay up to %d days.', 'mytravelstatus' ),
		...
		if ( count( $trips ) >= 3 ) {
			...
			if ( $pattern_insights ) {
				$suggestions[] = array(
					...
					'priority' => 'low',
					'icon'     => 'trending-up',
					'title'    => __( 'Your travel pattern', 'mytravelstatus' ),
		...
		if ( $days_used >= $settings['yellow_threshold'] && $days_used < 90 ) {
			...
			$suggestions[] = array(
				...
				'icon'     => 'shield-alert',
				'title'    => __( 'Approaching your limit', 'mytravelstatus' ),
				'message'  => sprintf(
					__( 'You have only %d days remaining. Plan carefully to avoid overstaying.', 'mytravelstatus' ),
					$buffer
				),
			);
		}
	...
	private function analyze_travel_patterns( array $trips ): ?string {
		...
		if ( $top_country ) {
			return sprintf(
				__( 'You visit %s most frequently (%d trips). Your average trip is %s days, primarily for %s.', 'mytravelstatus' ),
				$top_country,
	...
	private function get_user_settings( int $user_id ): array {
		...
		$saved = get_user_meta( $user_id, 'mts_settings', true );
	...
	private function validate_trip_dates( string $start_date, string $end_date ) {
		...
		if ( false === $start || false === $end ) {
			return new WP_Error(
				'invalid_date',
				__( 'Invalid date format. Use YYYY-MM-DD.', 'mytravelstatus' ),
				array( 'status' => 400 )
			);
		}
		...
		if ( $end < $start ) {
			return new WP_Error(
				'invalid_date_range',
				__( 'End date must be on or after start date.', 'mytravelstatus' ),
				array( 'status' => 400 )
			);
		}
		...
		if ( $days > 90 ) {
			return new WP_Error(
				'trip_too_long',
				__( 'A single trip cannot exceed 90 days.', 'mytravelstatus' ),
				array( 'status' => 400 )
			);
		}
...
<head>
	<meta charset="UTF-8">
	<title>' . esc_html__( 'MyTravelStatus Report', 'mytravelstatus' ) . '</title>
	<style>
...
<body>
	<div class="header">
		<h1>' . esc_html__( 'Schengen 90/180 Day Report', 'mytravelstatus' ) . '</h1>
		<div class="subtitle">' . esc_html( sprintf( __( 'Prepared for %s', 'mytravelstatus' ), $user->display_name ) ) . '</div>
		<div class="subtitle">' . esc_html( sprintf( __( 'Generated: %s', 'mytravelstatus' ), $data['generated_at'] ) ) . '</div>
	</div>

	<div class="summary">
		<div class="stat-card">
			<div class="stat-value">' . esc_html( $days_used ) . '</div>
			<div class="stat-label">' . esc_html__( 'Days Used', 'mytravelstatus' ) . '</div>
		</div>
		<div class="stat-card">
			<div class="stat-value">' . esc_html( $days_remaining ) . '</div>
			<div class="stat-label">' . esc_html__( 'Days Remaining', 'mytravelstatus' ) . '</div>
		</div>
		<div class="stat-card">
			<span class="status-badge" style="background-color: ' . esc_attr( $status_color ) . ';">' . esc_html( ucfirst( $status ) ) . '</span>
			<div class="stat-label" style="margin-top: 10px;">' . esc_html__( 'Current Status', 'mytravelstatus' ) . '</div>
		</div>
	</div>

	<div class="window-info">
		<p><strong>' . esc_html__( 'Current 180-Day Window:', 'mytravelstatus' ) . '</strong> ' . esc_html( $data['window_start'] ) . ' to ' . esc_html( $data['window_end'] ) . '</p>
	</div>

	<h2>' . esc_html__( 'Trip History', 'mytravelstatus' ) . '</h2>';

		if ( empty( $trips ) ) {
			$html .= '<p>' . esc_html__( 'No trips recorded.', 'mytravelstatus' ) . '</p>';
		} else {
			$html .= '
	<table class="trips-table">
		<thead>
			<tr>
				<th>' . esc_html__( 'Dates', 'mytravelstatus' ) . '</th>
				<th>' . esc_html__( 'Country', 'mytravelstatus' ) . '</th>
				<th>' . esc_html__( 'Duration', 'mytravelstatus' ) . '</th>
				<th>' . esc_html__( 'Category', 'mytravelstatus' ) . '</th>
				<th>' . esc_html__( 'Notes', 'mytravelstatus' ) . '</th>
			</tr>
		</thead>
		<tbody>';
			...
			<tr>
				...
				<td>' . esc_html( sprintf( _n( '%d day', '%d days', $duration, 'mytravelstatus' ), $duration ) ) . '</td>
		...
		$html .= '
	<div class="footer">
		<p>' . esc_html__( 'This report was generated by MyTravelStatus.', 'mytravelstatus' ) . '</p>
		<p>' . esc_html__( 'The 90/180 rule: Non-EU citizens may stay up to 90 days within any 180-day period in the Schengen area.', 'mytravelstatus' ) . '</p>
	</div>
</body>
</html>';
