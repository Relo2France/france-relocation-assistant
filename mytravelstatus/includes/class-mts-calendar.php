<?php
/**
 * Calendar Sync for MyTravelStatus.
 *
...
class MTS_Calendar {
	 ...
	 * @var string
	 */
	const CRON_HOOK = 'mts_calendar_sync';
	 ...
	 * @return MTS_Calendar
	 */
	public static function get_instance(): MTS_Calendar {
	...
	public function process_background_sync(): void {
		...
		$table = MTS_Schema::get_table( 'calendar_connections' );
		...
		foreach ( $connections as $connection ) {
			...
			if ( is_wp_error( $result ) ) {
				...
				if ( ! empty( $result['newEvents'] ) && $result['newEvents'] > 0 ) {
					 ...
					 */
					do_action( 'mts_new_calendar_events', $connection->user_id, $result['newEvents'], $result );
				}
			}
		 ...
		 */
		do_action( 'mts_calendar_sync_completed', $synced, $errors );
	...
	public function check_connection_permission( WP_REST_Request $request ) {
		...
		if ( ! $connection ) {
			return new WP_Error(
				'not_found',
				__( 'Connection not found.', 'mytravelstatus' ),
				array( 'status' => 404 )
			);
		}
		...
		if ( (int) $connection->user_id !== get_current_user_id() ) {
			return new WP_Error(
				'forbidden',
				__( 'You do not have permission to access this connection.', 'mytravelstatus' ),
				array( 'status' => 403 )
			);
		}
	...
	private function is_provider_configured( string $provider ): bool {
		if ( 'google' === $provider ) {
			return ! empty( get_option( 'mts_google_client_id' ) );
		}

		if ( 'microsoft' === $provider ) {
			return ! empty( get_option( 'mts_microsoft_client_id' ) );
		}
		...
		if ( ! $this->is_provider_configured( $provider ) ) {
			return new WP_Error(
				...
				sprintf(
					...
					__( '%s integration is not configured. Please contact the administrator.', 'mytravelstatus' ),
	...
	private function get_oauth_url( string $provider ): string {
		...
		set_transient( "mts_oauth_state_{$user_id}", $state, HOUR_IN_SECONDS );

		$redirect_uri = home_url( '/wp-json/mts/v1/calendar/callback' );

		if ( 'google' === $provider ) {
			$client_id = get_option( 'mts_google_client_id' );
		...
		if ( 'microsoft' === $provider ) {
			$client_id = get_option( 'mts_microsoft_client_id' );
	...
	public function handle_oauth_callback(): void {
		...
		// Verify state token.
		$stored_state = get_transient( "mts_oauth_state_{$user_id}" );
		...
		delete_transient( "mts_oauth_state_{$user_id}" );
	...
	private function exchange_code_for_tokens( string $provider, string $code ) {
		...
		if ( 'google' === $provider ) {
			$body['client_id']     = get_option( 'mts_google_client_id' );
			$body['client_secret'] = get_option( 'mts_google_client_secret' );
		} elseif ( 'microsoft' === $provider ) {
			$body['client_id']     = get_option( 'mts_microsoft_client_id' );
			$body['client_secret'] = get_option( 'mts_microsoft_client_secret' );
		}
	...
	public function disconnect( WP_REST_Request $request ) {
		...
		$events_table = MTS_Schema::get_table( 'calendar_events' );
	...
	private function process_events( $connection, array $events ): array {
		...
		$table        = MTS_Schema::get_table( 'calendar_events' );
		$detected     = 0;
		$new_events   = 0;
	...
	private function detect_travel( array $event ): array {
		...
		foreach ( self::COUNTRY_CODES as $country_name => $code ) {
			if ( strpos( $combined, $country_name ) !== false ) {
				...
				$result['is_schengen']  = MTS_Schema::is_schengen_country( $code );
		...
		foreach ( self::CITY_COUNTRIES as $city => $country ) {
			if ( strpos( $combined, $city ) !== false ) {
				...
				$result['is_schengen']  = MTS_Schema::is_schengen_country( $country );
	...
	public function import_events( WP_REST_Request $request ) {
		...
		$events_table = MTS_Schema::get_table( 'calendar_events' );
		$trips_table  = MTS_Schema::get_table( 'trips' );
		...
		foreach ( $event_ids as $event_id ) {
			...
			if ( $result ) {
				 ...
				 */
				do_action( 'mts_trip_imported_from_calendar', $user_id, $trip_id, $event_id );
			} else {
				$skipped++;
			}
		}
