...
The settings screen (member tools 2.8.0) has three tabs, Portal / Tools /
Members, and only renders what the portal reads. A new *tool* needs:

- [ ] `'menu_{item}' => true` in `$defaults`
- [ ] `'menu_{item}'` in `$bool_fields` in `sanitize_settings()`
- [ ] an entry in `tool_switches()` (name + one line on what it is) so the
      Tools tab shows its switch; anything not listed there is carried
      through as a hidden field

Labels, icons, section labels and menu order are no longer settings: names
and icons are product copy in `Sidebar.tsx`, and the rail's order is fixed.
