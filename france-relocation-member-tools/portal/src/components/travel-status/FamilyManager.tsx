...
const RELATIONSHIPS: { value: TravelStatusFamilyRelationship; label: string }[] = [
...
function getStatusColor(level: TravelStatusFamilyMemberStatus['level']) {
  switch (level) {
    case 'danger':
...
function StatusBadge({ status }: { status: TravelStatusFamilyMemberStatus }) {
  return (
...
function StatusBar({ status }: { status: TravelStatusFamilyMemberStatus }) {
  return (
...
interface MemberFormProps {
  member?: TravelStatusFamilyMember;
  onSave: (data: TravelStatusFamilyMemberCreate) => void;
  onCancel: () => void;
  isLoading: boolean;
}
...
function MemberForm({ member, onSave, onCancel, isLoading }: MemberFormProps) {
  ...
  const [relationship, setRelationship] = useState<TravelStatusFamilyRelationship | ''>(member?.relationship || '');
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          ...
          <select
            ...
            value={relationship}
            onChange={(e) => setRelationship(e.target.value as TravelStatusFamilyRelationship)}
            className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500"
          >
...
export default function FamilyManager({ className }: FamilyManagerProps) {
  ...
  const [editingMember, setEditingMember] = useState<TravelStatusFamilyMember | null>(null);

  const { data: membersData, isLoading: membersLoading, isError: membersError, refetch } = useTravelStatusFamilyMembers();
  const { data: summaryData, isLoading: summaryLoading } = useTravelStatusFamilySummary();

  const createMutation = useCreateTravelStatusFamilyMember();
  const updateMutation = useUpdateTravelStatusFamilyMember();
  const deleteMutation = useDeleteTravelStatusFamilyMember();

  const members = membersData?.members || [];
  const summary = summaryData;

  const handleCreate = async (data: TravelStatusFamilyMemberCreate) => {
    try {
  ...
  const handleUpdate = async (data: TravelStatusFamilyMemberCreate) => {
