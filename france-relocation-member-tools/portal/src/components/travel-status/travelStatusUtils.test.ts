...
function createTrip(
  startDate: string,
  endDate: string,
  country: TravelStatusCountry = 'France'
): TravelStatusTrip {
  return {
...
describe('getTravelStatusStatus', () => {
  it('returns safe for low usage', () => {
    expect(getTravelStatusStatus(0)).toBe('safe');
    expect(getTravelStatusStatus(30)).toBe('safe');
    expect(getTravelStatusStatus(59)).toBe('safe');
  });

  it('returns warning at yellow threshold', () => {
    expect(getTravelStatusStatus(60)).toBe('warning');
    expect(getTravelStatusStatus(79)).toBe('warning');
  });

  it('returns danger at red threshold', () => {
    expect(getTravelStatusStatus(80)).toBe('danger');
    expect(getTravelStatusStatus(89)).toBe('danger');
  });

  it('returns critical at or above 90 days', () => {
    expect(getTravelStatusStatus(90)).toBe('critical');
    expect(getTravelStatusStatus(100)).toBe('critical');
  });

  it('respects custom thresholds', () => {
    const thresholds = { yellow: 50, red: 70 };
    expect(getTravelStatusStatus(49, thresholds)).toBe('safe');
    expect(getTravelStatusStatus(50, thresholds)).toBe('warning');
    expect(getTravelStatusStatus(70, thresholds)).toBe('danger');
  });
});
...
describe('getTravelStatusSummary', () => {
  it('returns correct summary for no trips', () => {
    const summary = getTravelStatusSummary([]);
  ...
  it('returns correct summary with trips', () => {
    ...
    const summary = getTravelStatusSummary(trips);
