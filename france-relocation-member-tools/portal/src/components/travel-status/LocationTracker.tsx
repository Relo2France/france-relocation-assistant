...
export default function LocationTracker({ compact = false }: LocationTrackerProps) {
  ...
  const { data: todayStatus, isLoading: todayLoading } = useTravelStatusLocationToday();
  const storeLocationMutation = useStoreTravelStatusLocation();
...
function LocationHistoryView({ onClearRequest }: { onClearRequest: () => void }) {
  const { data, isLoading, refetch, isRefetching } = useTravelStatusLocationHistory({ limit: 50 });
  const deleteMutation = useDeleteTravelStatusLocation();
...
function ClearHistoryConfirm({
  ...
  const clearMutation = useClearTravelStatusLocationHistory();
