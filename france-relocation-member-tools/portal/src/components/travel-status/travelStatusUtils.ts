 ...
 */
export function getTravelStatusStatus(
  daysUsed: number,
 ...
 */
export function getTravelStatusSummary(
  trips: TravelStatusTrip[],
  thresholds: { yellow: number; red: number } = { yellow: 60, red: 80 }
): TravelStatusSummary {
  ...
  return {
    ...
    status: getTravelStatusStatus(daysUsed, thresholds),
...
export function wouldTripViolate(
  existingTrips: TravelStatusTrip[],
  newTripStart: string,
  newTripEnd: string
): TravelStatusPlanningResult {
  ...
  while (current <= tripEnd) {
    ...
    const hypotheticalTrip: TravelStatusTrip = {
      id: 'temp',
