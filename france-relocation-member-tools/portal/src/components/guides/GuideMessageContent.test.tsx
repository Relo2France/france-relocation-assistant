...
describe('GuideMessageContent', () => {
  it('renders a rule as a section break, never as literal text', () => {
    const { container } = render(
      <GuideMessageContent content={'Some text.\n\n---\n\nMore text.'} />
    );
  ...
  it('absorbs a rule that precedes a heading into one section break', () => {
    const { container } = render(
      <GuideMessageContent content={'Body.\n\n---\n\n**In Practice**\n\n- A tip'} />
    );
    expect(container.textContent).not.toContain('---');
  ...
  it('joins wrapped lines into a single paragraph', () => {
    const { container } = render(
      <GuideMessageContent content={'French pharmacies are easily\nidentified by the green\ncross sign outside.'} />
    );
  ...
  it('renders every heading level, including h1', () => {
    const { container } = render(
      <GuideMessageContent content={'# Choosing a Mutuelle\n\n## Structure\n\n### Detail'} />
    );
    expect(container.textContent).not.toContain('#');
  ...
  it('renders bold, italic and code without leaking markers', () => {
    const { container } = render(
      <GuideMessageContent content={'The **mutuelle** covers the *ticket modérateur* via `PUMA`.'} />
    );
  ...
  it('keeps accented French text intact', () => {
    const { container } = render(
      <GuideMessageContent content={'Sécurité Sociale reimburses 70-80%.'} />
    );
  ...
  it('renders bullet and numbered lists', () => {
    const { container } = render(
      <GuideMessageContent content={'- First\n- Second\n\n1. One\n2. Two'} />
    );
  ...
  it('folds a wrapped list item back into its bullet', () => {
    const { container } = render(
      <GuideMessageContent content={'- Pharmacists are highly trained\n  and play a clinical role.'} />
    );
  ...
  it('only linkifies http(s) targets', () => {
    const { container } = render(
      <GuideMessageContent content={'[safe](https://service-public.fr) and [bad](javascript:alert(1))'} />
    );
