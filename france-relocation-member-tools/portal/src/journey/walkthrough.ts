...
export function walkthroughFor(stage: JourneyStageId, ctx: WalkContext): Walkthrough {
  ...
  switch (stage) {
    ...
    case 'prepare': {
      const civil = { title: 'Certified copies of your civil records', why: `Birth certificate${married && withSpouse ? ', and the marriage certificate' : ''}, ordered fresh from the state. A photocopy from the drawer will not be apostilled.`, done: firstKnown(T(/civil records|gather (all )?required documents|birth certificate/i)) };
      ...
      const apostille = { title: `Apostilles from ${birthState}`, why: 'The Secretary of State that issued a record certifies it. One to three weeks in most states, and it cannot start until the copies are in hand: this sets the pace of everything after it.', done: firstKnown(D('birth-certificate-apostilled', 'marriage-certificate'), T(/apostill/i)) };
